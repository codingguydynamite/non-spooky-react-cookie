...
- [x] **Texts from a CMS (2026-09-23)**. `null` in `texts` keeps the built-in text, `itemsLabel` is a plain trigger label with the count in parentheses (no plural forms, so all texts are plain JSON), new `DeepPartialNullable` type, `getBuiltInTexts` and `BUILT_IN_LANGUAGES` from both entries, right-to-left switch and text alignment. Driven by wiring the banner into a Payload project with eleven locales. Tests in `tests/resolve-texts.test.ts`; playground scenario `CmsSuppliedTexts.tsx`.
